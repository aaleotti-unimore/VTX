...
\begin{tikzpicture}[auto, node distance=10em]
	\node [input](input){};
	\node [box, right of=input](clas){Classificatore Neurale};
	...
	\path [line] (gan) -- node[right]{Adversarial Examples} (clas);
	\path [line] (clas) -- (output)node[near end,name=out]{Previsione};
\end{tikzpicture}
%\end{document}