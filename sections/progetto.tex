...
Il generatore aggiunge in testa alla stuttura del decoder un layer denso che mappa un input randomico in una codifica di dominio, come mostrato in figura \ref{fig:generator}

\begin{figure}[htb]
    \centering
	\includegraphics[width=0.7\columnwidth]{figures/generator.tex}
	\caption{Struttura del generatore}
\label{fig:generator}
\end{figure}

Il discriminatore implementa in uscita un layer denso che esegua regressione logistica sui domini codificati dalla sottorete encoder. \ref{fig:discriminator}


\begin{figure}[htb]
    \centering
	\includegraphics[width=0.7\columnwidth]{figures/discriminator.tex}
	\caption{Struttura del discriminatore}
\label{fig:discriminator}
\end{figure}


Rispetto al lavoro presentato in \cite{deepdga}, si è deciso di implementare una architettura più semplificata. Come mostrato in \cite{1606.03498} le GAN sono tipicamente difficoltose da allenare, in quanto l'uso di funzioni di costo porta ad un difficile \textit{tuning} per ottenere un equilibrio di Nash tra le due parti in gioco.

\todo{trattare input e output}
