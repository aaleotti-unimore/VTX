...
La Rete Convoluzionale è formata da un layer convoluzionale, in grado di cogliere pattern locali all'interno dei domini, con l'obiettivo di rilevare n-grammi significativi presenti nei domini reali e poterli successivamente riprodurre in maniera sintetica all'interno della GAN. 
Parte integrante di una rete convoluzionale è il livello di \textit{pooling}, in cui le features piu importanti del livello precedente vengono ridotte in un tensore di dimensione secondo il valor medio. La decisione di utilizzare average pooling anziche max pooling è stata presa a causa dell'elevata instabilità intrinseca alle GAN, per le quali max pooling è un fattore contribuente. 
...
Seconda fase della sottorete Encoder è la presenza di una Long Short-term Memory Network. 
<cos'è una lstm>
<perchè usarla>
