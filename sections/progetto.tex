...
La struttura del \textit{MLP} in esame è stata raggiunta dopo una serie di test pratici in cui si sono messi a confronto tre modelli differenti di per numero di neuroni all'interno degli \textit{hidden layer}: 
\begin{itemize}
...
\item un modello intermedio composto da un layer di input con un numero di neuroni pari alla dimensione delle stringhe codificate, un layer intermedio di dimensione 128, un layer di dimensione minore a 64 ed un layer finale di dimensione 1.
\end{itemize}

I tre modelli messi a confronto hanno mostrato comportamenti simili ma in diverse situazioni è emerso il modello intermedio
