...
Come mostrato in \cite{1508.06615} le reti convoluzionali sono in grado di cogliere le \textit{features} più importanti in una sequenza di caratteri. Nel modello utilizzato si indichi $\mathbb{V}$ come vocabolario dei caratteri ammissibili per i domini di rete, $d$ la dimensione dei domini codificati, and $\mathbf{Q} \in \mathbb{R}^{d \times |\mathcal{C}|}$  la matrice di codifica dei caratteri. Sia supposto che il dominio $k \in \mathcal{V}$ sia composto da una sequenza di caratteri $[c_1, \dots, c_{l}]$, dove $l$ è la lunghezza del dominio $k$.
Allora la rappresentazione a livello di caratteri di $k$ è data dalla matrice $\mathbf{C}^k \in \mathbb{R}^{d \times l}$, dove la $j$ma colonna corrisponde alla codifica dei caratteri per $c_j$.
Si applichi una convoluzione tra $\mathbf{C}^k$  ed un \textit{fitro} (o \textit{kernel}) $\mathbf{H} \in \mathbb{R}^{d \times w}$ di larghezza $w$, al quale viene aggiunto un \textit{bias} ed una non linearità per ottenere una \textit{feature map} $\mathbf{f}^k \in \mathbb{R}^{l - w + 1}$. In particolare l'$i$mo elemento di $\mathbf{f}^k$ è dato da:
$$\mathbf{f}^k[i] = \mbox{tanh}(\langle \mathbf{C}^k[\ast, i:i+w-1], \mathbf{H} \rangle + b)$$

where $\mathbf{C}^k[\ast, i:i+w-1]$ è la colonna da $i$ ad $(i+w-1)$ di $\mathbf{C}^k$ ed $$ \langle \mathbf{A}, \mathbf{B} \rangle = \Tr(\mathbf{A}\mathbf{B}^T)$$ è il prodotto interno di Frobenius.
